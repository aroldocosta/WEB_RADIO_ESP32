...
#define MQTT_BROKER_URI       "mqtt://mqtt.oficinabr.com"
#define MQTT_DEFAULT_USER     "oficinabr"
#define MQTT_DEFAULT_PASS     "oficinabr123"
...
static void mqtt_event_handler(void *handler_args, esp_event_base_t base, int32_t event_id, void *event_data)
{
    ...
    case MQTT_EVENT_DATA:
        ESP_LOGI(TAG, "Notificacao MQTT recebida no topico: %.*s", event->topic_len, event->topic);
        ESP_LOGI(TAG, "Conteudo: %.*s", event->data_len, event->data);
        /* Aqui o firmware detecta atualizações de URLs do catálogo emitidas pela plataforma */
        break;
...
esp_err_t mqtt_client_app_init(void)
{
    ...
    char client_id[32];
    snprintf(client_id, sizeof(client_id), "benedictatu_%02x%02x%02x", mac[3], mac[4], mac[5]);
